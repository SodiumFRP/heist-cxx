...
namespace Set23_Impl {
    ...
    {
        return heist::fmap<heist::list<Position>, Iterator>(
            [] (heist::list<Position> stack) {
                return Iterator(stack);
            },
            boost::apply_visitor(LowerBoundVisitor(compare, heist::list<Position>(), a), n)
        );
    }
    ...
    bool isTwoNode(const Node& node)
    {
        return boost::get<Leaf1>(&node.n) != NULL ||
    ...
    )
    {
        ...
        if (const Node2* parent1 = boost::get<Node2>(&top.node.n)) {
            ...
            switch (top.ix) {
                case 0:
                    if (isTwoNode(*(const Node*)parent.q.value)) {  // case 3(a)
                        return boost::make_optional(
                            Iterator(
                                Position(
                                    Node(Node2(
                ...
                case 2:
                    if (isTwoNode(*(const Node*)parent.p.value)) {  // case 3(a)
                        return boost::make_optional(
                            Iterator(
                                Position(
                                    Node(Node2(
                ...
                case 4:
                    if (isTwoNode(*(const Node*)parent.q.value)) {  // case 3(b)
                        return boost::make_optional(
                            Iterator(
                                Position(
                                    Node(Node2(
